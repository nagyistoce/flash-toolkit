		...
		foreach ( $settings as $key => $setting ) {
			...
			switch ( $setting['type'] ) {
				...
				case 'icon_picker' :
					...
					<div id="tg-widget-icon-picker" class="flash-icon <?php echo esc_attr( $class ); ?>">
						<p><label for="<?php echo $field_id; ?>"><?php echo esc_html( $setting['label'] ); ?></label></p>
						<select class="widefat flash-enhanced-select-icons" style="width: 100%" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo $field_name; ?>" data-placeholder="<?php esc_attr_e( 'Choose icons&hellip;', 'flash-toolkit' ); ?>" title="<?php esc_attr_e( 'Icon', 'flash-toolkit' ) ?>">
							<option value=""></option>
							<?php foreach ( $setting['options'] as $option_key => $option_value ) : ?>
								<option value="<?php echo esc_attr( $option_key ); ?>" data-icon="<?php echo esc_attr( $option_key ); ?>" <?php selected( $option_key, $value ); ?>><?php echo esc_html( $option_value ); ?></option>
							<?php endforeach; ?>
